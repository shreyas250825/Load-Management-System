...
def check_alerts(data):
    ...
    if data["voltage"] > voltage_threshold:
        ...
        decrease_threshold("voltage")
    if data["current"] > current_threshold:
        alert_message = f"Current exceeded! Current: {data['current']} A"
        threading.Thread(target=speak_alert, args=(alert_message,)).start()
        messagebox.showwarning("Alert", alert_message)
        decrease_threshold("current")
    if data["power"] > power_threshold:
        alert_message = f"Power exceeded! Current: {data['power']} W"
        threading.Thread(target=speak_alert, args=(alert_message,)).start()
        messagebox.showwarning("Alert", alert_message)
        decrease_threshold("power")

# Decrease the threshold values for voltage, current, and power
def decrease_threshold(param):
    if param == "voltage":
        voltage_threshold = float(voltage_threshold_entry.get()) - 5
        voltage_threshold_entry.delete(0, tk.END)
        voltage_threshold_entry.insert(0, str(voltage_threshold))
    elif param == "current":
        current_threshold = float(current_threshold_entry.get()) - 0.5
        current_threshold_entry.delete(0, tk.END)
        current_threshold_entry.insert(0, str(current_threshold))
    elif param == "power":
        power_threshold = float(power_threshold_entry.get()) - 50
        power_threshold_entry.delete(0, tk.END)
        power_threshold_entry.insert(0, str(power_threshold))
